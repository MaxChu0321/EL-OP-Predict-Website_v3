...
import { useJobStore_ER_OP_pre } from 'stores/job'
import { useTestCase_ER_OP_pre } from '../helpers/useDemo'
import DownloadExampleButton from './DownloadExampleButton.vue';

export default defineComponent({
  name: 'CaseForm_ER_OP_pre',
  components: {
  ...
  setup (props, { emit }) {
    const job_store = useJobStore_ER_OP_pre()
    const { initialValues } = toRefs(props)
    let { demo_case } = useTestCase_ER_OP_pre()
