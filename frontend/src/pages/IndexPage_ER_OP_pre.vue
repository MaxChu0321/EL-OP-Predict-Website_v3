    ...
    <div class="row justify-center">
      <q-card v-if="!showResults" class="q-pa-md column justify-center" flat bordered>
        ...
        <q-tab-panels v-model="mode" animated>
          <q-tab-panel name="case">
            <CaseForm_ER_OP_pre @formSubmitted="(data) => handleFormSubmission('early recurrence', data)" />
          </q-tab-panel>
      ...
      <div v-if="showResults" class="flex-container full-width">
        <div class="col form-container" align="center">
          ...
          <q-tab-panels v-model="mode" animated>
            <q-tab-panel name="case">
              <CaseForm_ER_OP_pre class="form-part" :initial-values="formValues" @formSubmitted="(data) => handleFormSubmission('early recurrence', data)" />
            </q-tab-panel>
            <q-tab-panel name="batch">
              <BatchForm_ER_OP_pre />
            </q-tab-panel>
        ...
        <CaseResultPage_ER_OP_pre class="result-part" />
      </div>
    </div>
  </q-page>
</template>
...
import CaseForm_ER_OP_pre from "src/components/CaseForm_ER_OP_pre.vue"
import BatchForm_ER_OP_pre from 'src/components/BatchForm_ER_OP_pre.vue'
import CaseResultPage_ER_OP_pre from "src/pages/CaseResultPage_ER_OP_pre.vue"

export default defineComponent({
  name: 'IndexPage_ER_OP_pre',
  components: {
    CaseForm_ER_OP_pre,
    BatchForm_ER_OP_pre,
    CaseResultPage_ER_OP_pre
  },
  setup() {
    const mode = ref('case')
